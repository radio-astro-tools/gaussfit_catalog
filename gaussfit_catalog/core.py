...
def gaussfit_catalog(fitsfile, region_list, radius=1.0*u.arcsec,
    ...
    for ii,reg in enumerate(region_list):
        ...
        fit_data[sourcename] = {'amplitude': result.amplitude,
                                ...
                                'e_amplitude': fit_info['param_cov'][0,0]**0.5 if success else None,
                                'e_center_x': fit_info['param_cov'][1,1]**0.5*u.deg if success else None,
                                'e_center_y': fit_info['param_cov'][2,2]**0.5*u.deg if success else None,
                                'e_fwhm_x': fit_info['param_cov'][3,3]**0.5 * STDDEV_TO_FWHM * pixscale.to(u.arcsec) if success else None,
                                'e_fwhm_y': fit_info['param_cov'][4,4]**0.5 * STDDEV_TO_FWHM * pixscale.to(u.arcsec) if success else None,
                                'e_pa': fit_info['param_cov'][5,5]**0.5 * u.deg if success else None,
                                'success': success,
                               }
